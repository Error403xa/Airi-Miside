...
const statusLabelKey = computed(() => `settings.pages.modules.messaging-telegram.status.${statusKind.value}`)
const statusDetail = computed(() => t(`settings.pages.modules.messaging-telegram.status-detail.${statusKind.value}`))
...
const lastSyncText = computed(() => lastSyncAt.value ? new Date(lastSyncAt.value).toLocaleTimeString() : t('settings.pages.modules.messaging-telegram.never'))
const lastStatusText = computed(() => lastStatusAt.value ? new Date(lastStatusAt.value).toLocaleTimeString() : t('settings.pages.modules.messaging-telegram.never'))
const errorDetail = computed(() => {
  if (lastError.value.includes('@proj-airi/server-runtime'))
    return t('settings.pages.modules.messaging-telegram.errors.server-runtime')
  if (lastError.value.includes('@proj-airi/telegram-bot'))
    return t('settings.pages.modules.messaging-telegram.errors.telegram-service')
  ...
  <div flex="~ col gap-6">
    ...
    <div>
      <Button
        ...
        variant="primary"
        @click="telegramStore.saveSettings()"
      />
    </div>
      ...
      <div :class="['flex items-start gap-3']">
        ...
        <div :class="['min-w-0 flex-1']">
          ...
          <div :class="['mt-3 flex flex-wrap items-center gap-3 text-xs opacity-70']">
            <span>{{ t('settings.pages.modules.messaging-telegram.last-sync', { time: lastSyncText }) }}</span>
            <span>{{ t('settings.pages.modules.messaging-telegram.last-status', { time: lastStatusText }) }}</span>
          </div>
        </div>

        <Button
          :label="t('settings.pages.modules.messaging-telegram.retry')"
          icon="i-lucide-refresh-cw"
          variant="secondary"
          size="sm"
          :loading="isConnecting"
          @click="telegramStore.retryConnection()"
        />
      </div>
    </div>
  </div>
