...
type TelegramStatusKind = 'disabled' | 'missing-token' | 'idle' | 'syncing' | 'waiting-for-service' | 'connecting' | 'ready' | 'error'
...
export const useTelegramStore = defineStore('telegram', () => {
  ...
  const enabled = useLocalStorageManualReset<boolean>('settings/telegram/enabled', false)
  const token = useLocalStorageManualReset<string>('settings/telegram/token', '')
  const statusKind = ref<TelegramStatusKind>('idle')
  ...
  async function syncSettingsToBackend() {
    ...
    if (!enabled.value) {
      ...
      statusMessage.value = 'Telegram integration is disabled.'
    }
    else if (!configured.value) {
      statusKind.value = 'missing-token'
      statusMessage.value = 'Enter a Telegram bot token to connect.'
    }
    else {
      statusKind.value = 'syncing'
      statusMessage.value = 'Sending Telegram configuration to the local bot service.'
    }

    try {
      await configurator.updateFor('telegram', {
        token: token.value,
      ...
      statusMessage.value = 'Waiting for Telegram bot service to validate the token.'
      startStatusTimeout()
    }
    catch (error) {
      statusKind.value = 'error'
      statusMessage.value = 'Failed to send Telegram configuration.'
      lastError.value = error instanceof Error ? error.message : String(error)
      console.error('Failed to synchronize Telegram settings:', error)
    }
  }
    ...
    statusTimeout = setTimeout(() => {
      ...
      if (statusKind.value === 'waiting-for-service' || statusKind.value === 'syncing') {
        ...
        statusMessage.value = serverConnected.value
          ? 'Telegram bot service did not report back.'
          : 'Local messaging server is not connected.'
        lastError.value = serverConnected.value
          ? 'Check that @proj-airi/telegram-bot is running, then retry.'
  ...
  function handleModuleStatus(event: ModuleStatusEvent) {
    ...
    const isTelegram = identity?.plugin?.id === 'telegram' || source?.plugin?.id === 'telegram'
    if (!isTelegram)
      return
