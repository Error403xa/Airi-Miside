...
const providerId = 'airi-official-audio-speech'
const defaultModel = 'stepfun/stepaudio-2.5-tts'
const defaultVoice = 'yuanqishaonv'
...
const voice = computed({
  get: () => providers.value[providerId]?.voice || defaultVoice,
  set: (value) => {
    if (!providers.value[providerId])
      providers.value[providerId] = {}
...
watch(
  ...
  (newConfig) => {
    if (newConfig) {
      ...
      if (!config.voice && voice.value !== defaultVoice)
...
<template>
  <SpeechProviderSettings
    ...
    placeholder="eyJhbGci... (airi.moeru.ai 登录 token)"
  >
    <!-- Voice settings specific to AIRI official gateway -->
    <template #voice-settings>
      <!-- Model input -->
      <FieldInput
        v-model="model"
        label="Model"
        description="TTS model served by the AIRI official gateway"
        placeholder="stepfun/stepaudio-2.5-tts"
      />
