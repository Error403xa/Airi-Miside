...
class ModelClient:
    ...
    def request_stream(
        ...
        """
        Send a request to the model and stream thinking deltas.

        Args:
            messages: List of message dictionaries in OpenAI format.
            on_thinking_delta: Optional callback for streaming thinking text.
        ...
        for chunk in stream:
            ...
            if chunk.choices[0].delta.content is not None:
                ...
                for marker in action_markers:
                    if marker in buffer:
                        thinking_part = buffer.split(marker, 1)[0]
                        if thinking_part and on_thinking_delta:
                            on_thinking_delta(thinking_part)
                        in_action_phase = True
                        marker_found = True
                        break
                ...
                if not is_potential_marker:
                    if buffer and on_thinking_delta:
                        on_thinking_delta(buffer)
                    buffer = ""

        if buffer and not in_action_phase and on_thinking_delta:
            on_thinking_delta(buffer)
