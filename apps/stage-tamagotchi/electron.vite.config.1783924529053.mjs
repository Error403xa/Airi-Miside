...
let stageUIAssetsRoot = resolve(join(import.meta.dirname, '..', '..', 'packages', 'stage-ui', 'src', 'assets'))
let sharedCacheDir = resolve(join(import.meta.dirname, '..', '..', '.cache'))
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          createReadStream(assetPath).on('error', next).pipe(res)
        }
        catch (error) {
...
let electron_vite_config_default = defineConfig({
  main: {
    plugins: [
      Info(),
      {
